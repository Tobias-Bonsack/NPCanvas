...
- **Schema versioning.** `schemaVersion` is a literal type, current **10**. To evolve: add `ProjectFileVN`, widen `StoredProjectFile`, point `ProjectFile` at it, branch in `readProjectFile`, migrate forward on load. Migrations chain one step at a time (`migrateV9(migrateV8(...migrateV1(...)))`) — one new function and case per version. **Ordering trap:** a versioned file type naming the *current* profile (or similar) type silently redefines what an old document held once that type's shape changes — re-point the old `ProjectFileVN` at the frozen pre-migration type. Per-migration specifics are one-line comments at each `migrateVN` in `src/project/data-file-migrations.ts`.
